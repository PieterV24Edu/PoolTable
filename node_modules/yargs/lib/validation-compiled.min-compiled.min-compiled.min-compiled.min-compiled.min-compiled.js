...
var _typeof5 = typeof Symbol === "function" && _typeof6(Symbol.iterator) === "symbol" ? function (obj) {
  ...
  var self = {};self.nonOptionCount = function (argv) {
    var demanded = yargs.getDemanded();if (demanded._ && argv._.length < demanded._.count) {
    ...
    var options = yargs.getOptions(),
        defaultValues = [true, false, ""];if (options.requiresArg.length > 0) {
      var missingRequiredArgs = [];options.requiresArg.forEach(function (key) {
        var value = argv[key];if (~defaultValues.indexOf(value) || Array.isArray(value) && !value.length) {
        ...
        var message = "Missing argument values: " + missingRequiredArgs.join(", ");usage.fail(message);
      }
    }
  };self.requiredArguments = function (argv) {
    var demanded = yargs.getDemanded(),
        missing = null;Object.keys(demanded).forEach(function (key) {
      if (!argv.hasOwnProperty(key)) {
        missing = missing || {};missing[key] = demanded[key];
      }
    });if (missing) {
      var customMsgs = [];Object.keys(missing).forEach(function (key) {
        var msg = missing[key].msg;if (msg && customMsgs.indexOf(msg) < 0) {
          customMsgs.push(msg);
        }
    ...
    var descriptions = usage.getDescriptions(),
        ...
        unknown = [],
        aliasLookup = {};Object.keys(aliases).forEach(function (key) {
  ...
  };var checks = [];self.check = function (f) {
    checks.push(f);
  };self.customChecks = function (argv, aliases) {
    checks.forEach(function (f) {
      try {
        var result = f(argv, aliases);if (!result) {
  ...
  };var implied = {};self.implies = function (key, value) {
    ...
    var implyFail = [];Object.keys(implied).forEach(function (key) {
      var num,
          origKey = key,
          value = implied[key];num = Number(key);key = isNaN(num) ? key : num;if (typeof key === "number") {
        key = argv._.length >= key;
      } else if (key.match(/^--no-.+/)) {
        key = key.match(/^--no-(.+)/)[1];key = !argv[key];
      } else {
        key = argv[key];
      }num = Number(value);value = isNaN(num) ? value : num;if (typeof value === "number") {
        value = argv._.length >= value;
      } else if (value.match(/^--no-.+/)) {
        value = value.match(/^--no-(.+)/)[1];value = !argv[value];
      } else {
        value = argv[value];
      }if (key && !value) {
...
//# sourceMappingURL=validation-compiled.min-compiled.min-compiled.min-compiled.min-compiled.min-compiled.js.map