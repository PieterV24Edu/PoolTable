...
var acorn;
var screw_ie8 = true;
var ARGS = yargs.usage("$0 input1.js [input2.js ...] [options]\n\
...
the source map and the output file.").describe("o", "Output file (default STDOUT).").describe("b", "Beautify output/specify output options.").describe("m", "Mangle names/pass mangler options.").describe("r", "Reserved names to exclude from mangling.").describe("c", "Enable compressor/pass compressor options. \
Pass options like -c hoist_vars=false,if_return=false. \
Use -c with no argument to use the default compression options.").describe("d", "Global definitions").describe("e", "Embed everything in a big function, with a configurable parameter/argument list.").describe("comments", "Preserve copyright comments in the output. \
...
if (ARGS.mangle_props == 2) {
    ...
    if (COMPRESS && !("properties" in COMPRESS)) COMPRESS.properties = false;
}
...
if (BEAUTIFY) UglifyJS.merge(OUTPUT_OPTIONS, BEAUTIFY);
...
if (process.platform === "win32") files = UglifyJS.simple_glob(files);
...
async.eachLimit(files, 1, function (file, cb) {
    read_whole_file(file, function (err, code) {
        ...
        time_it("parse", function () {
            if (ARGS.spidermonkey) {
                ...
                if (!TOPLEVEL) TOPLEVEL = program;else TOPLEVEL.body = TOPLEVEL.body.concat(program.body);
            } else if (ARGS.acorn) {
    ...
    if (ARGS.dump_spidermonkey_ast) {
        ...
        if (SOURCE_MAP) {
            ...
            var source_map_url = ARGS.source_map_url || (P_RELATIVE ? path.relative(path.dirname(OUTPUT_FILE), ARGS.source_map) : ARGS.source_map);
    ...
    if (x !== "") {
        ...
        ast.walk(new UglifyJS.TreeWalker(function (node) {
            ...
            if (node instanceof UglifyJS.AST_Assign) {
                ...
                if (constants) value = new Function("return (" + value.print_to_string() + ")")();
                ret[name] = value;
...
function time_it(name, cont) {
    ...
    if (ARGS.stats) {
        ...
        if (STATS[name]) STATS[name] += spent;else STATS[name] = spent;
    }
    return ret;
}
...
//# sourceMappingURL=uglifyjs-compiled.js.map