...
var players = [], currentPlayer, otherPlayer;
...
var startPosArray;
var collisionArray = [], collisionCount;
var ballShot = false;
...
function init() {
    ...
    ballArray = [
        new PlayBall(0, 0, poolTable.children),
        new PoolBall(1, 1),
        new PoolBall(2, 1),
        new PoolBall(3, 1),
        new PoolBall(4, 1),
...
function render() {
    ...
    for(let i = 0; i < ballArray.length; i++)
    {
        for(let j = i + 1; j < ballArray.length; j++)
        {
            if(ballArray[i].inScene && ballArray[j].inScene) {
...
function checkMovingBalls() {
    for(let i = 0; i < ballArray.length; i++)
    {
        if(ballArray[i].speed > 0 && ballArray[i].inScene)
            return true;
    }
    return false;
}
